...
# currentFileName = 'GraphFusionModelV14_0'
...
n_units_pre = [n_hidden_units + addFeatureLength + 1, n_hidden_units*2, n_hidden_units, n_hidden_units/2, predictLength]
...
with autoEncoderGraph.as_default():
    ...
    with tf.variable_scope('encoder_fusionGraph'):
        # channel 1
        demandGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        demandGraphMatrixWeight = tf.Variable(tf.random_normal([stationNumber, 1]))

        # channel 2
        distGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        distGraphMatrixWeight = tf.Variable(tf.random_normal([stationNumber, 1]))

        graphWeightMatrix = tf.concat([demandGraphMatrixWeight, distGraphMatrixWeight], 1)

        graphWeightMatrixSoftMax = tf.nn.softmax(graphWeightMatrix)

        demandGraphMatrixTensor = tf.multiply(tf.reshape(graphWeightMatrixSoftMax[:, 0], [-1, 1]), demandGraphMatrixTensorRaw)
        distGraphMatrixTensor = tf.multiply(tf.reshape(graphWeightMatrixSoftMax[:, 1], [-1, 1]), distGraphMatrixTensorRaw)

        # fusion graph
        fusionGraphMatrixWeight = tf.Variable(tf.random_normal([stationNumber, 1]))
        fusionGraphMatrixTensor = tf.multiply((demandGraphMatrixTensor + distGraphMatrixTensor), fusionGraphMatrixWeight)
        ...
        encoderInput = tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), fusionGraphMatrixTensor)

        encoderInput = tf.reshape(encoderInput, [-1, n_steps_encoder, n_inputs])
...
with preNNGraph.as_default():
    ...
    inputFeature = tf.concat([inputEmbeddingFeature, otherFeature, preNNInput], 1)
        ...
        for iteration in range(currentIterations):
            ...
            trainLoss, _ = autoEncoderSession.run([loss_autoEncoder, trainOperation_autoEncoder],
                                    feed_dict={
                                        ...
                                        demandGraphMatrixTensorRaw: demandGraphMatrixFeed,
